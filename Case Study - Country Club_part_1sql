...
SELECT f.name AS facility_name, CONCAT(m.firstname, ' ', m.surname) AS member_name,  
	CASE WHEN m.surname = 'GUEST' AND (f.guestcost * b.slots > 30) THEN f.guestcost * b.slots
         WHEN m.surname != 'GUEST' AND (f.membercost * b.slots > 30) THEN f.membercost * b.slots 
    END AS cost
FROM Bookings AS b   
INNER JOIN Facilities AS f
   ON b.facid = f.facid
...
SELECT name AS facility_name, member_name, SUM(slots) AS 'usage'
FROM
...
SELECT name AS facility_name, month, SUM(slots) AS 'usage'
FROM
